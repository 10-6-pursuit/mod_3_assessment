...
const {checkName, validateURL, checkDescription} = require("../validations/checkAnime")
...
animes.get("/", async (req, res) => {
  try {
    const listOfAnime = await getAllAnimes();
    if (listOfAnime[0]) {
      res.status(200).json(listOfAnime)
    } else {
...
animes.get("/:id", async (req, res) => {
  ...
  if (oneAnime) {
    ...
    console.error({error: "anime not found"});
  }
})
//create one anime
animes.post("/", checkName, checkDescription, async (req, res) => {
  try {
    ...
    return res.status(500).json({error: "notfound"});
  }
} )
...
// }
animes.put("/:id", checkDescription, checkName, async (req, res) => {
  const {id} = req.params;
  try {
