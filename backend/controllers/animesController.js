...
const { checkName } = require("../validation/validation.js");
...
animes.put("/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const updatedOneAnime = await updateOneAnime(id);
    if (updatedOneAnime) {
      res.status(200).json(updatedOneAnime);
    } else {
...
animes.delete("/:id", async (req, res) => {
  ...
  try {
  const { id } = req.params;
  const deletedOneAnime = await deleteOneAnime(id);
  if (deletedOneAnime) {
    res.status(200).json(deletedOneAnime);
  } else {
    res.status(404).json("Anime not found");
  }
}catch(error){
