...
const { checkNameAndDescription } = require("../middlewares/validations");
...
animes.post("/", checkNameAndDescription, async (req, res) => {
	try {
		const createdAnime = await createOneAnime(
			req.body.name,
...
animes.put("/:animeId", checkNameAndDescription, async (req, res) => {
	const { animeId } = req.params;
	try {
		const updatedAnime = await updateOneAnime(animeId, req.body);
		// console.log(updatedAnime, "when database error or");
		if (updatedAnime) {
			res.status(200).json(updatedAnime);
		} else {
			// console.log(updatedAnime, "not found else");
			res.status(404).send({ error: "Not found" });
		}
	} catch (error) {
		// console.log(error, "update errir");
